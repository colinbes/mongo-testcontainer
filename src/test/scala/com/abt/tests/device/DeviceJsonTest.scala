...
class DeviceJsonTest
  ...
  "Json Write/Read Devices" should "have equal strings" in {
    val list = deviceList
    val devices = read[List[Device]](list)
    devices.length must equal(9)
    val deviceCollection = MongoT.getCollection[Document]("tst-device")
    devices.foreach(device => {
      val dao: String = write(device)
      val doc: Document = Document.apply(dao)
      wait(deviceCollection.withWriteConcern(WriteConcern.ACKNOWLEDGED).insertOne(doc).toFuture())
    })
    val readDevices = wait(
      deviceCollection
        .find()
        .map(item => {
